...
#define FORMATTED_JSON_MEASUREMENT "{\"device_id\":\"%s\", \"event_num\":%d, \"hand\": { \"thump\": { \"acc\": { \"x\":%.3f, \"y\":%.3f, \"z\":%.3f }, \"gyro\": { \"x\":%.3f, \"y\":%.3f, \"z\":%.3f }, \"inclination\": { \"roll\":%.3f, \"pitch\":%.3f, \"yaw\":%.3f } } } }\n"
...
void sendEvent(){
  ...
  */
  ...
  const int buffer_size= 1 + snprintf(NULL, 0, FORMATTED_JSON_MEASUREMENT, 
  mov.device_id.c_str(), mov.event_num, 
  mov.hand.thump.acc.x, mov.hand.thump.acc.y, mov.hand.thump.acc.z, 
  mov.hand.thump.gyro.x, mov.hand.thump.gyro.y, mov.hand.thump.gyro.z,
  mov.hand.thump.inclination.roll,mov.hand.thump.inclination.pitch, mov.hand.thump.inclination.yaw);
  Serial.print("  buffer_size: ");Serial.print(buffer_size);
  assert(buffer_size > 0);
  char package[buffer_size];
  int size_written = snprintf(package, buffer_size, FORMATTED_JSON_MEASUREMENT, 
  mov.device_id.c_str(), mov.event_num, 
  mov.hand.thump.acc.x, mov.hand.thump.acc.y, mov.hand.thump.acc.z, 
  mov.hand.thump.gyro.x, mov.hand.thump.gyro.y, mov.hand.thump.gyro.z,
  mov.hand.thump.inclination.roll,mov.hand.thump.inclination.pitch, mov.hand.thump.inclination.yaw);
